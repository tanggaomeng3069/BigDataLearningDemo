...
public class KeySecret {
    ...
    // 秘钥
    private static final String PASSWORD_CRYPT_KEY = "zhengzhou";
     ...
     * @return
     */
    public static byte[] encryptMode1(byte[] originalText) {
        try {
            // 生成秘钥
     ...
     *
     */
    public static byte[] decryptMode1(byte[] cipherText) {
        try {
            // 生成秘钥
            SecretKey deskey = new SecretKeySpec(build3DesKey(PASSWORD_CRYPT_KEY), "AES");
            // 实例化负责解密
...
//			if (n<secretMsg.length - 1);
    ...
    public static String getDecryptMode(String str) {
        byte[] bt = toBytes(str);
        return new String(decryptMode(bt));
    }

    public static String getEncryptMode(String str) {
        byte[] secretStr = encryptMode(str.getBytes());
        return byte2Hex(secretStr);
    }
...
//        System.out.println("[加密后]:" + new String(secretStr));
//        System.out.println("[加密后]:" + secretStr);
//        System.out.println("[加密后]:" + str);
//        // 解密
//        byte[] ad = KeySecret.toBytes(str);
//        System.out.println("[转换后的bytes]: " + new String(ad));
//        byte[] myMsgArr = KeySecret.decryptMode(ad);
...
//        System.out.println("[解密后]: " + myMsgArr);
        ...
        // AES CBC加解密 成功
        String msg = "lyang123456789034256Superuser";
        byte[] secretStr = KeySecret.encryptMode1(msg.getBytes("UTF-8"));
        String str = KeySecret.byte2Hex(secretStr);
        System.out.println("[加密后]:" + str);
        byte[] ad = KeySecret.toBytes(str);
        byte[] myMsgArr1 = KeySecret.decryptMode1(ad);
