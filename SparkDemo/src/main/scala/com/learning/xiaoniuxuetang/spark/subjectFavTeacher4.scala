package com.learning.xiaoniuxuetang.spark
...
/**
  ...
  * Describe:
  */
object subjectFavTeacher4 {

  def main(args: Array[String]): Unit = {

    // 初始化配置
    val conf: SparkConf = new SparkConf().setAppName("subjectFavTeacher4").setMaster("local[4]")
    ...
    // 计算有多少学科
    val subjects: Array[String] = subjectTeacherAndOne.map((_: ((String, String), Int))._1._1).distinct().collect()

    // 自定义一个分区器，并且按照指定的分区器进行分区
    val sbPartitioner = new SubjectParitioner2(subjects)

    // 聚合，将学科和老师联合当作key
    val reduced: RDD[((String, String), Int)] = subjectTeacherAndOne.reduceByKey(sbPartitioner, (_: Int) + (_: Int))
