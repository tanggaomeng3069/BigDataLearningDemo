...
/**
 * HDFS文件内容格式为：1,Ambari,V2.7.3,CentOS,7.4.1708,bigdata@ambari.com
 */
object ReadHDFSWriteHBase {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val spark: SparkSession = SparkSession
            .builder()
            .appName("ReadHDFSWriteHBase")
            .master("local[*]")
            .getOrCreate()
        val sc: SparkContext = spark.sparkContext

        val dataRDD: RDD[String] = sc.textFile(KerberosConfigLoader.getString("hdfs.write.hbase.path"))
        ...
        dataRDD.foreachPartition(partition => {
            ...
            partition.foreach(line => {
                ...
                put.addColumn(Bytes.toBytes(tableColumnFamily), Bytes.toBytes("action"), Bytes.toBytes(field(0)))
                put.addColumn(Bytes.toBytes(tableColumnFamily), Bytes.toBytes("name"), Bytes.toBytes(field(0)))
                put.addColumn(Bytes.toBytes(tableColumnFamily), Bytes.toBytes("version"), Bytes.toBytes(field(1)))
                put.addColumn(Bytes.toBytes(tableColumnFamily), Bytes.toBytes("os"), Bytes.toBytes(field(2)))
                put.addColumn(Bytes.toBytes(tableColumnFamily), Bytes.toBytes("Core"), Bytes.toBytes(field(2)))
                put.addColumn(Bytes.toBytes(tableColumnFamily), Bytes.toBytes("mail"), Bytes.toBytes(field(2)))
